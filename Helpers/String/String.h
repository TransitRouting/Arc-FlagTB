...
inline std::string secToTime(double t, bool displaySeconds = false)
{
    return secToTime<int>(t, std::numeric_limits<int>::max(), displaySeconds);
}
...
{
    if (time.size() != 8)
        error("The string " + time + " is not in the format YYYYMMDD");
