...
protected:
    IdMapping(const size_t n = 0)
        : std::vector<size_t>(n)
    {
    }
    IdMapping(const std::vector<size_t>& data)
...
public:
    Order(const size_t n = 0)
        : IdMapping(n)
    {
    }
    explicit Order(const std::vector<size_t>& data)
        : IdMapping(data)
    {
    }
    explicit Order(std::vector<size_t>&& data)
        : IdMapping(std::move(data))
    {
    }
