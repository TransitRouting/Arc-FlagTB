...
public:
    ...
    // Arc-Flag TB
    inline int getNumberOfPartitionCells() const noexcept {return raptorData.getNumberOfPartitionCells();}
    ...
    inline void writeHypMETISFile(const std::string& fileName, const bool verbose = true) noexcept {
        if (verbose) std::cout << "Start creating HypMETIS file " << fileName << " ...\n";
        ...
        for (RouteId route : routes) {
            AssertMsg(firstTripOfRoute[route + 1] - firstTripOfRoute[route] == tripsOfRoute(route).size(), "HypMETIS file not correctly weighted!");
        ...
        if (verbose) std::cout << "Finished creating HypMETIS file " << fileName << "!\n";
    }
