            ...
            for (const Edge first : graph.edgesFrom(from)) {
                for (const Edge second : graph.edgesFrom(graph.get(ToVertex, first))) {
                    if (graph.get(ToVertex, second) != to)
                        continue;
                    if (graph.get(attributeName, first) + graph.get(attributeName, second) < graph.get(attributeName, edge))
                        return false;
                }
            }
        }
...
inline void printInfo(const GRAPH& graph, std::ostream& out = std::cout) noexcept
{
    ...
    if (!edgeData.empty())
        out << "    Edges contain: " << edgeData << "." << std::endl;
    if (vertexData.empty() && edgeData.empty())
        out << "      no additional data exists." << std::endl;
}
