...
inline void toEdgeListCSV(const std::string& fileBaseName, const GRAPH& graph) noexcept
{
    ...
    csv << "FromStopEventId,ToStopEventId\n";
    for (const auto [edge, from] : graph.edgesWithFromVertex()) {
...
template <typename GRAPH>
inline void fromStrasserBinary(const std::string& fileName, GRAPH& graph, const double timeFactor = 1,
    const double distanceFactor = 1, const double coordinateFactor = 1) noexcept
{
    ...
    for (const Vertex vertex : graph.vertices()) {
        if constexpr (GRAPH::HasVertexAttribute(Coordinates)) {
            graph.set(Coordinates, vertex,
                Geometry::Point(Construct::LatLong, latitude[vertex] * coordinateFactor,
