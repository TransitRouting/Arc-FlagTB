...
public:
    struct CutEdge {
        ...
        inline bool operator<(const CutEdge& e) const noexcept
        {
            return std::tie(from, to, edge) < std::tie(e.from, e.to, e.edge);
        }

        friend std::ostream& operator<<(std::ostream& out, const CutEdge& t)
        {
            return out << t.from << ", " << t.to << ", " << t.edge << std::endl;
        }
...
public:
    Dinic(DynamicFlowGraph&& dynamicGraph)
        ...
        , edges(dynamicGraph.numVertices(), noEdge)
...
public:
    ...
    inline void isolateVertex(const Vertex vertex)
    {
        for (const Edge edge : graph.edgesFrom(vertex)) {
    ...
    inline void removeCut(const std::vector<CutEdge>& cut)
    {
        ...
        while (!nextQueue.empty()) {
            ...
            while (!currentQueue.empty()) {
                ...
                for (const Edge edge : graph.edgesFrom(from)) {
                    if (FORWARD) {
                        if (residualFlows[edge] <= capacityLimit)
                            continue;
                    } else {
                        if (residualFlows[graph.get(ReverseEdge, edge)] <= capacityLimit)
                            continue;
                    }
                    const Vertex to = graph.get(ToVertex, edge);
                    if (distances[to] != intMax)
                        continue;
                    distances[to] = distance;
                    nextQueue.emplace_back(to);
                    activeVertices.insert(to);
                    if (FORWARD) {
                        if (targetVertices.contains(to))
                            connected = true;
                    } else {
                        if (sourceVertices.contains(to))
                            connected = true;
                    }
                }
            }
    ...
    inline size_t findBlockingPath()
    {
        ...
        while (!openVertices.empty()) {
            ...
            if (edges[from] == graph.beginEdgeFrom(from)) {
                ...
                edges[from]--;
                if (residualFlows[edges[from]] <= capacityLimit)
                    continue;
                ...
                if (edges[to] == graph.beginEdgeFrom(to))
                    continue;
