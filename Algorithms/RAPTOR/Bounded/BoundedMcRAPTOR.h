...
public:
    ...
    inline void run(const StopId source, const int departureTime, const StopId target, const double arrivalSlack,
...
private:
    ...
    {
        AssertMsg(data.isStop(stop), "Stop " << stop << " is out of range!");
        if (-backwardPruningRAPTOR.getArrivalTimeByRoute(stop, maxTrips - currentNumberOfTrips()) < label.arrivalTime)
            return;
    ...
    {
        AssertMsg(data.isStop(stop), "Stop " << stop << " is out of range!");
        if (-backwardPruningRAPTOR.getArrivalTimeByTransfer(stop, maxTrips - currentNumberOfTrips()) < label.arrivalTime)
            return;
        if (bestLabelsByTransfer[targetStop].dominates(label))
            return;
