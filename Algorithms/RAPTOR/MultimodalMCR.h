...
public:
    struct Label {
        ...
        Label(const Label& parentLabel, const StopId stop, const size_t parentIndex, const int extraTime = 0)
    ...
    struct DijkstraLabel {
        ...
        DijkstraLabel(const int departureTime, const StopId sourceStop, const size_t mode)
        ...
        DijkstraLabel(const DijkstraLabel& parentLabel, const size_t mode, const int travelTime = 0)
        ...
        DijkstraLabel(const Label& parentLabel, const StopId parentStop, const size_t parentIndex, const size_t mode,
            const int travelTime = 0)
            : arrivalTime(parentLabel.arrivalTime + TransferModeOverhead[mode] + travelTime)
            , parentStop(parentStop)
            , parentIndex(parentIndex)
        {
            std::copy(std::begin(parentLabel.transferTime), std::end(parentLabel.transferTime),
...
public:
    ...
    inline void clear() noexcept
    {
        ...
        if constexpr (RESET_CAPACITIES) {
            ...
            for (size_t i = 0; i < NumTransferModes; i++) {
                std::vector<DijkstraBagType>(dijkstraBags[i].size()).swap(dijkstraBags[i]);
            }
        } else {
            rounds.clear();
...
private:
    ...
    {
        const DijkstraLabel sourceLabel(sourceDepartureTime, StopId(sourceVertex), mode);
        ...
        for (const Vertex stop : initialTransfers[mode].getForwardPOIs()) {
            ...
            const DijkstraLabel dijkstraLabel(sourceLabel, mode, initialTransfers[mode].getForwardDistance(stop));
        ...
        if (initialTransfers[mode].getDistance() != INFTY) {
            const DijkstraLabel targetLabel(sourceLabel, mode, initialTransfers[mode].getDistance());
    ...
    {
        AssertMsg(queue[mode].empty(), "Queue still has " << queue[mode].size() << " elements!");
        for (const StopId stop : stopsUpdatedByRoute) {
            if (initialTransfers[mode].getBackwardDistance(stop) == INFTY)
                continue;
            const BagType& bag = previousRound()[stop];
            for (size_t i = 0; i < bag.size(); i++) {
                const DijkstraLabel targetLabel(bag[i], stop, i, mode,
                    initialTransfers[mode].getBackwardDistance(stop));
                if (dijkstraBags[mode][targetVertex].template merge<false>(targetLabel)) {
                    arrivalByTransfer(targetStop, targetLabel);
                }
            }
        }
        ...
        while (!queue[mode].empty()) {
            ...
            for (const Edge edge : getTransferGraph(mode).edgesFrom(u)) {
                ...
                if (v == targetVertex || v == uLabel.parentStop)
                    continue;
                profiler.countMetric(METRIC_EDGES);
                const DijkstraLabel vLabel(uLabel, mode, getTransferGraph(mode).get(TravelTime, edge));
    ...
    inline bool arrivalByEdge(const size_t mode, const Vertex vertex, const DijkstraLabel& label) noexcept
    {
        ...
        if constexpr (TargetPruning)
            if (checkTargetPruning(label))
                return false;
        if (!dijkstraBags[mode][vertex].template merge<true>(label))
            return false;
    ...
    inline void arrivalByTransfer(const StopId stop, const LABEL& label) noexcept
    {
        ...
        const int parentDepartureTime = (label.parentStop == sourceVertex)
            ? sourceDepartureTime
