...
private:
    struct Label {
        ...
        Label(const Label& parentLabel, const size_t mode, const int travelTime, const StopId stop,
            const size_t parentIndex, const Edge transferId, const StopEventId stopEvent)
        ...
        Label(const Label& parentLabel, const int travelTime, const StopId stop, const size_t parentIndex,
            const Edge transferId, const StopEventId stopEvent)
            : arrivalTime(parentLabel.arrivalTime + travelTime)
            , parentStop(stop)
            , parentIndex(parentIndex)
            , parentDepartureTime(parentLabel.arrivalTime)
        ...
        Label(const int arrivalTime, const Label& parentLabel, const StopId parentStop, const size_t parentIndex,
            const int parentDepartureTime, const RouteId route, const StopEventId stopEvent)
...
public:
    MultimodalUBMHydRA(const TripBased::MultimodalData& data, const TripBased::Data& forwardPruningData,
        const TripBased::Data& backwardPruningData, const TransferGraph& backwardTransitiveGraph,
        const std::vector<CH::CH>& chData, const Profiler& profilerTemplate = Profiler())
        : data(data)
        ...
        , multimodalInitialTransfers(transitiveInitialTransfers, initialTransfers, data.modes, chData[0].numVertices(),
              raptorData.numberOfStops())
...
private:
    ...
    inline void collectRoutesServingUpdatedStops() noexcept
    {
        for (const StopId stop : stopsUpdatedByTransfer) {
            for (const RouteSegment& route : raptorData.routesContainingStop(stop)) {
                ...
                AssertMsg(raptorData.stopIds[raptorData.firstStopIdOfRoute[route.routeId] + route.stopIndex] == stop,
        ...
        for (const RouteId route : routesServingUpdatedStops.getKeys()) {
            ...
            while (stopIndex < tripSize - 1) {
                ...
                for (const RouteLabel& label : routeBag.labels) {
                    if (label.lastExitIndex + 1 < stopIndex)
                        continue;
        ...
        for (const StopId stop : stopsUpdatedByTransfer) {
            for (size_t i = 0; i < previousRound()[stop].size(); i++) {
                ...
                for (; stopIndex < StopIndex(reachedIndex + 1); stopIndex++, stopEvent++) {
                    ...
                    arrivalByRoute(toStop,
                        Label(arrivalTime, label, enterStop, i, parentDepartureTime, route, stopEvent));
                }
            }
        }
    }
    ...
    inline void arrivalByRoute(const StopId stop, const Label& label) noexcept
    {
        ...
        if (checkTargetPruning(label))
            return;
        if (!bestBagByRoute[stop].mergeWithStrongDominance(BestLabel(label)))
            return;
        if (!currentRound()[stop].mergeUndominatedUnlessEqual(label))
            return;
    ...
    inline void arrivalByTransfer(const StopId stop, const Label& label) noexcept
    {
        ...
        if (checkTargetPruning(label))
            return;
        if (!bestBagByTransfer[stop].mergeWithStrongDominance(BestLabel(label)))
            return;
        if (!currentRound()[stop].mergeUndominatedUnlessEqual(label))
            return;
        profiler.countMetric(METRIC_STOPS_BY_TRANSFER);
        if (raptorData.isStop(stop))
    ...
    inline void targetArrivalByTransfer(const Label& label) noexcept
    {
        if (-backwardPruningQuery.getDepartureTime(maxTrips - currentNumberOfTrips()) < label.arrivalTime)
            return;
        if (!bestBagByTransfer[targetStop].merge(BestLabel(label)))
            return;
        profiler.countMetric(METRIC_STOPS_BY_TRANSFER);
