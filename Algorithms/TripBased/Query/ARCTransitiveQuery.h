...
class ARCTransitiveQuery {

public:
    using Profiler = PROFILER;
    using Type = ARCTransitiveQuery<Profiler>;
...
private:
    ...
    struct EdgeLabel {
        EdgeLabel(const StopEventId stopEvent = noStopEvent, const TripId trip = noTripId, const StopEventId firstEvent = noStopEvent, const std::vector<bool> arcFlags = {}) :
            stopEvent(stopEvent),
            trip(trip),
...
private:
    ...
    inline void enqueue(const Edge edge, const size_t parent) noexcept {
        ...
        if (compressed) {
            if (!compressedFlags[compressedIndizes[edge]][targetFlag] || reachedIndex.alreadyReached(label.trip, label.stopEvent - label.firstEvent)) return;
        } else {
            if (!label.arcFlags[targetFlag] || reachedIndex.alreadyReached(label.trip, label.stopEvent - label.firstEvent)) return;
        }
